...
<head>
    <meta charset="UTF-8">
    <title>Halaman Data Mata Pelajaran</title>
            ...
            <form method="POST" action="">
                <div class="row mb-3">
                    ...
                    <div class="col-md-4 d-flex align-items-end justify-content-end">
                        <button type="submit" name="action" value="tampilkan_data" class="btn btn-primary mr-2">Tampilkan Data</button>
                        <button type="submit" name="action" value="edit_presensi" class="btn btn-secondary">Edit Presensi</button>
                    </div>
                </div>
            </form>
                ...
                if ($kelas) {
                    if (isset($_POST['action']) && $_POST['action'] == 'tampilkan_data') {
                        ...
                        if ($result->num_rows > 0) {
                            echo '<form method="POST" action="proses/update_presensi.php">';
                        ...
                        $sql = "SELECT * FROM absen WHERE kelas = ?";
                        $stmt = $conn->prepare($sql);
                        $stmt->bind_param("s", $kelas);
                        $stmt->execute();
                        $result = $stmt->get_result();
                        ...
                        if ($result->num_rows > 0) {
                            ...
                            echo '<p>Tidak ada presensi di kelas ini.</p>';
                        }
